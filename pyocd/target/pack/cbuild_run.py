...
class TraceConfig:
    ...
    def __init__(
        ...
        self._server_ports: Set[int] = set()
        ...
        for entry in debugger.get('trace') or []:
            ...
            server_port = entry.get('server-port', 5555) if mode == 'server' else None
            if server_port is not None:
                if server_port in self._server_ports:
                    LOG.warning("Ignoring trace configuration with duplicate server port %d", server_port)
                    continue
                self._server_ports.add(server_port)
            file = None
            if mode == 'file':
...
class CbuildRunDebugSequenceDelegate(DebugSequenceDelegate):
    ...
    def trace_enabled(self) -> bool:
        """@brief Whether cbuild-run enables trace capture debug sequences."""
